...
class DocumentRepository implements DocumentInterface
{
    public function getAllDocuments()
    {
        ...
        if (Auth::user()->role->id == 3) {
            $user = Auth::user();
            $documents = $user->document()->where('status', '0')->orderBy('id', 'DESC')->get();
            return $documents;
        }
    ...
    {
        // Display only pending documents for admin
        if (Auth::user()->role->id == 3) {
            $user = Auth::user();
            $pendingDocs = $user->document()->where('status', '1')->orderBy('id', 'DESC')->get();
            return $pendingDocs;
        }

        // Display only pending documents
        $pendingDocs = Document::where('status', '1')->orderBy('id', 'DESC')->get();
        return $pendingDocs;
    }
